...
The command builds the selected Nx projects, uses `pnpm pack` so `workspace:` ranges receive their publish-time rewrite,
canonicalizes the packed `package.json` key order, and writes content-addressed tarballs plus `uapkg-bundle.json`. The
manifest has no timestamp and is deterministic for the same consumer manifest, source identity, requested ref, and
package contents.
