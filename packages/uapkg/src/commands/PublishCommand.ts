...
export class PublishCommand implements Command {
  ...
  public async execute(): Promise<number> {
    try {
      ...
      const kind = await this.resolvePublishKind(trust.alias, manifest.name);
      ...
      const scopedOwner = scopedOwnerName(manifest.name);
      ...
      if (scopedOwner && configuredOwner && configuredOwner.toLowerCase() !== scopedOwner.toLowerCase()) {
        throw new Error(
          `Package "${manifest.name}" belongs to the "${scopedOwner}" namespace; --owner cannot select "${configuredOwner}".`,
        );
      }
      const owner = scopedOwner ?? configuredOwner;
      if (kind === 'publish_new_package' && authentication.kind === 'oidc') {
        throw new Error(
          'GitHub Actions OIDC cannot create a package. Publish the first version interactively, then configure an exact-package trusted publisher.',
        );
      }
      if (kind === 'publish_new_package' && authentication.kind === 'login' && !owner) {
        throw new Error(
          'An unscoped package needs an owner for its first publication. Use --owner or set publish.owner in uapkg.json.',
        );
      }
      ...
      const submission: RegistryRequestSubmission = {
        ...
        kind,
        ...(kind === 'publish_new_package' && owner ? { ownerOrganizationName: owner } : {}),
        payload: {
      ...
      const created = await client.submitRegistryRequest(authentication.credential, submission, authentication.otp);
  ...
  private async resolvePublishKind(
    registryAlias: string,
    packageName: string,
  ): Promise<'publish_new_package' | 'publish_new_version'> {
    const registryResult = this.root.registryCore.getOrCreateRegistry(registryAlias);
    if (!registryResult.ok) {
      throw new Error(registryResult.diagnostics[0]?.message ?? `Registry "${registryAlias}" is unavailable.`);
    }
    const existing = await registryResult.value.getPackageManifest(packageName, registryAlias);
    if (existing.ok) return 'publish_new_version';
    if (existing.diagnostics.every((diagnostic) => diagnostic.code === 'PACKAGE_NOT_FOUND')) {
      return 'publish_new_package';
    }
    throw new Error(existing.diagnostics[0]?.message ?? `Unable to inspect package "${packageName}".`);
  }
  ...
  private async resolveSource(
    ...
    const pathToManifest = normalizeManifestPath(
      this.options.manifestPath ?? manifestPublish?.manifestPath ?? (await this.deriveManifestPath()),
    );
  ...
  private async deriveManifestPath(): Promise<string> {
    const git = (this.root.config.get('git') as string | null) ?? 'git';
    try {
      ...
      const relativePath = relative(gitRoot, join(this.root.cwd, 'uapkg.json')).replaceAll('\\', '/');
      return normalizeManifestPath(relativePath);
    } catch (error) {
      throw new Error('Unable to derive the repository-relative uapkg.json path. Use --manifest-path.', {
        cause: error,
      });
    }
  }
...
function normalizeManifestPath(input: string): string {
  ...
  if (!value || isAbsolute(value) || value.startsWith('/') || value.split('/').includes('..')) {
    throw new Error('The publish manifest path must be a safe repository-relative path.');
  }
  return value;
}
