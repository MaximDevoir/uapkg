...
/**
 * Lightweight C# structural parser tailored to Unreal build files.
 *
 * We do NOT invoke the Roslyn / csharp-ast graph for analysis because the
 * pipeline only needs three anchors:
 *   1. The single class inheriting `ModuleRules` / `TargetRules`.
 *   2. Its constructor matching `(ModuleRules|TargetRules)(Type Target) : base(Target)`.
 *   3. The body braces of (1) and (2) so injectors can splice content.
 *
 * Lighter-than-AST parsing keeps this file < 200 LOC and avoids pulling a
 * heavyweight runtime into `uapkg install`.
 */
export class CSharpStructureAnalyzer {
  public parseModule(filePath: string, source: string): ParsedCSharpFile {
    return this.parseFile('module', filePath, source, 'ModuleRules', 'ReadOnlyTargetRules');
  }

  public parseTarget(filePath: string, source: string): ParsedCSharpFile {
  ...
  private parseFile(
    ...
    return { kind, classInfo, constructorInfo, source };
  }
  ...
  private findConstructor(
    ...
    return { signatureIndex, bodyOpenBraceIndex, bodyCloseBraceIndex };
  }
  ...
  private findMatchingBrace(source: string, openIndex: number): number {
    ...
    for (let i = openIndex; i < source.length; i += 1) {
      ...
      if (inMultiLineComment) {
        if (prev === '*' && current === '/') inMultiLineComment = false;
        continue;
      }
      if (inSingleQuote) {
        if (current === "'" && prev !== '\\') inSingleQuote = false;
        continue;
      }
      if (inDoubleQuote) {
